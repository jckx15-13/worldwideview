...
so it downloads alongside hydration rather than after it.

> Superseded 2026-08-17 by "Phase 41 results" below. The waterfall theory was
> correct and the saving is no longer inferred — it was measured at **308 ms**,
> then eliminated.
...
**Decided 2026-08-17: `cpus` stays at 2.** More parallel webpack workers means
more concurrent heap against `Dockerfile:81`'s 3072 MB ceiling, and local peak
build RSS is already 3,575 MB. The earlier justification for `cpus: 2` (OOM
mitigation) was refuted, but the setting turns out to be right for a different
and now-documented reason. Build wall-clock is a developer cost, not a user one;
trading a container OOM for it is a bad deal. Revisit only alongside raising
that `--max_old_space_size`, and validate in Docker.
...
## Phase 41 results (measured 2026-08-17)

Phase 41 was blocked on the fact that this project could not observe its own
runtime. Two tools closed that gap, and the waterfall fix followed from what
they showed.
...
|---|---|---|
| `@next/bundle-analyzer` | `pnpm analyze` | Per-module bundle attribution, replacing the First Load JS columns Next 16 removed |
| Production perf harness | `pnpm perf` | What a real browser downloads, and when |

The harness (`playwright.perf.config.ts` + `tests/perf/globe-load.perf.spec.ts`)
exists because every other Playwright config runs `pnpm dev`. Dev-server numbers
are unminified, unsplit and compiled on demand — worthless for bundle work. This
one runs `next start` against a real build.

It needs no database: `src/proxy.ts` gates `/` on session-cookie **presence**
only (`hasBetterAuthCookie`, explicitly documented as not verifying validity)
and `src/app/page.tsx` does no auth of its own, so a synthetic cookie reaches the
identical client bundle. Chunk hashes are read from
`.next/react-loadable-manifest.json` rather than hardcoded.

### The waterfall: measured, then fixed

`globeDiscoveryMs` is the wall-clock moment the browser *first requests* a
GlobeView chunk. Anything after `firstJsRequestMs` is idle network on the
critical path.
...
| Globe chunks referenced in HTML | 0 of 6 | **6 of 6** | fixed |
| `rel="preload"` links in HTML | 2 | 8 | +6 |
...
| Total JS bytes (encoded) | 2,073,831 | 2,073,831 | unchanged |

**The fix ships zero additional bytes.** It is purely a scheduling change:
`src/app/page.tsx` calls React 19's `preload()` with the hrefs from
`src/lib/globe-preload.ts`, so the chunks appear in the prerendered HTML and the
browser's preload scanner starts them immediately instead of waiting for
hydration to render `<GlobeView />`.

**Two failure modes were checked, not assumed:**
...
2. *Clean builds.* `/` is prerendered at build time, so if the manifest were not
   written before prerendering, the preload would silently do nothing — in
   ...
   `.next/server/app/index.html`, 8 preload links.
...
- **Localhost, not a real network.** The 308 ms gap is hydration-bound and would
  persist on any connection, but the 569 ms download saving is a lower bound
  locally and would grow on slower links, where 1.34 MB takes real time.
- **`appReadyMs` is unmeasured (`null`).** The boot sequence never completes in
  ...
  backends into the harness would make time-to-interactive measurable; that is
  the obvious next increment.
- **Per-route First Load JS is still not emitted by Next 16.** `pnpm analyze`
  now supersedes it for attribution purposes.

### Still open

- **Draco.** The dominant chunk carries `draco` ×87 alongside `Cesium` ×87, but
  whether that is the decoder itself or just loader plumbing is **unverified** —
  Cesium also loads Draco as a worker from `public/cesium/`. `pnpm analyze` can
  now settle it; splitting it before that would be a guess.
